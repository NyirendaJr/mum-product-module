        ...
        <v-card class="pt-3">
            <div class="d-flex flex-row">
                <div class="flex-grow-1 pa-2">
                    <v-btn
                        @click="$router.push({name:'stock.product.create', params: {stockId: compStockId}})"
            ...
            <div class="d-flex flex-lg-row flex-sm-column">
                <div class="flex-grow-1 pa-2">
                    <v-text-field filled prepend-inner-icon="search" label="Filter By Name" v-model="filters.name"></v-text-field>
                </div>
                <div class="flex-grow-1 pa-2">
                ...
                <tr>
                    <th v-for="header in headers">
                        <div v-if="header.value=='mfg'" :class="`text-${header.align}`">
                            <v-icon>event</v-icon> {{header.text}}
                        </div>
                ...
                <tr v-for="item in items" :key="item.id">
                    <td>
                        <div class="d-flex align-content-space-around flex-wrap">
                            <v-btn @click="$router.push({name:'stock.product.edit' ,params:{productId: item.id, stockId: compStockId}})" class="pa-1" small outlined color="info">
                                <!--<v-icon small>mdi-pencil</v-icon>-->
                                Edit
                            </v-btn>
                            <v-btn @click="trash(item)" class="pa-1 ml-2" small outlined color="red">
        ...
        data () {
            return {
                headers: [
                    ...
                    { text: 'ProductCategory', value: 'category', align: 'left', sortable: false },
                    ...
                    //{ text: 'Date added', value: 'created_at', align: 'left', sortable: false },
            ...
            trash (product) {
                ...
                self.$store.commit('showDialog', {
                    ...
                    okCb: () => {
                        ...
                        axios.delete('/mauzo/products/' + product.id).then(response => {
